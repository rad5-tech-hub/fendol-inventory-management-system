...
export default function UseStoreModal({ show, store, onClose, onSuccess }) {
  ...
  const handleBlur = (field) => {
    ...
    const value = field === 'pondId' ? pondId : quantityUsed;
    const err = validate(field, value);
    setErrors((prev) => {
      const copy = { ...prev };
      if (err) copy[field] = err;
  ...
  const handleSubmit = async (e) => {
    ...
    const allTouched = { pondId: true, quantityUsed: true };
      ...
      >
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className={styles.body}>
            ...
            <div className={styles.field}>
              <label className={styles.label}>
                Quantity {`(${store?.unit || 'units'})`}<span className={styles.required}>*</span>
              </label>
              <input
                ref={qtyRef}
